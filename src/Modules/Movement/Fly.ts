...
async function antiFly(player: Player, now: number) {
    ...
    if (
        playerPrevPos === undefined ||
        (player.isOnGround && velocityY === 0) ||
        (velocityY < 0 && player.location.y < previousLocations.get(id)?.y)
    ) {
    ...
    //@ts-expect-error
    if (
        (player.threwTridentAt && now - player.threwTridentAt < 2000) ||
    ...
    const jumpBoostEffect =
        player.getEffect(MinecraftEffectTypes.JumpBoost)?.amplifier ?? 0;

    if (jumpBoostEffect >= 4) return;

    const didFindSlime: boolean = blockAround(
        player,
    ...
    if (
        player.hasTag("matrix:trident") &&
        !player.hasTag("matrix:using_item")
    ) {
        player.removeTag(`matrix:trident`);
    }
    if (
        velocityY > config.antiFly.maxVelocityY &&
        !player.hasTag("matrix:slime")
    ) {
        if (velocityY === Math.abs(velocityY) || skipCheck.get(id) > 0) return;

        const prevLoc = previousLocations.get(id);
        flag(player, "Fly", config.antiFly.maxVL, config.antiFly.punishment, [
...
async function antiNofall(player: Player) {
    ...
    if (player.isFlying || player.isInWater || player.hasTag("matrix:joined"))
        return;

    if (
        !isOnGround &&
        velocity.y === 0 &&
        Math.hypot(velocity.x, velocity.z) > 0
    ) {
        const prevLoc = previousLocations.get(id);
        flag(
            player,
...
system.runInterval(() => {
    const toggle: boolean = (world.getDynamicProperty("antiNofall") ??
        config.antiNofall.enabled) as boolean;
...
system.runInterval(() => {
    const toggle: boolean = (world.getDynamicProperty("antiFly") ??
        config.antiFly.enabled) as boolean;
    if (toggle !== true) return;
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
    ...
    system.runTimeout(
        () => player.removeTag("matrix:joined"),
        config.antiFly.skipCheck
    );
});
